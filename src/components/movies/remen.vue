<template>
  <div class="container">
    <el-breadcrumb separator="/">
      ...
      <el-breadcrumb-item>热门列表</el-breadcrumb-item>
    </el-breadcrumb>
    <!-- <el-upload
      class="avatar-uploader"
      action="http://localhost:3000/api/remen/imgse/"
    ...
    >
      ...
      >
        <el-form-item label="电影id" prop="movie_id">
          <el-input :disabled="true" v-model="remenForm.movie_id"></el-input>
        </el-form-item>
        <el-form-item label="电影名" prop="movie_name">
          <el-input v-model="remenForm.movie_name"></el-input>
        </el-form-item>
        <el-form-item label="类型" prop="tag">
          <el-input v-model="remenForm.tag"></el-input>
        </el-form-item>
        <el-form-item label="电影信息" prop="movie_info">
          <el-input
            v-model="remenForm.movie_info"
            type="textarea"
            maxlength="255"
        ...
        <el-form-item label="评价人数">
          <el-input
            v-model="remenForm.people_number"
            :disabled="true"
          ></el-input>
        </el-form-item>
        <el-form-item label="评分">
          <el-input v-model="remenForm.rate" :disabled="true"></el-input>
        </el-form-item>
      </el-form>
      <span slot="footer" class="dialog-footer">
        <el-button type="primary" @click="remenEditSave">保 存</el-button>
      </span>
    </el-dialog>
...
export default {
  ...
  methods: {
    ...
    handleEdit(index, row) {
      this.remenEdit = true;
      this.remenForm.movie_id = row.movie_id;
      this.remenForm.movie_name = row.movie_name;
      this.remenForm.movie_info = row.movie_info;
      this.remenForm.people_number = row.people_number;
      this.remenForm.rate = row.rate;
      this.remenForm.tag = row.tag;
    ...
    handleClose(done) {
      this.$confirm("还未保存，是否关闭？")
        .then((_) => {
          ...
          this.remenForm.movie_name = "";
          this.remenForm.movie_id = "";
          this.remenForm.movie_info = "";
          this.remenForm.tag = "";
          this.remenForm.people_number = "";
          this.remenForm.rate = "";
        })
        .catch((_) => {});
    },
    ...
    remenEditSave() {
      if (!this.edit_insert) {
        this.axios({
          ...
          params: {
            img: "",
            movie_name: this.remenForm.movie_name,
            movie_info: this.remenForm.movie_info,
            tag: this.remenForm.tag,
            movie_id: this.remenForm.movie_id,
          },
        }).then((res) => {
          if (res.data.code == 200) {
            this.$message.success(res.data.msg);
            this.remenEdit = false;
            this.$router.go(0);
          } else {
